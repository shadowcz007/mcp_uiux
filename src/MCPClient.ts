...
import { Resource, ResourceTemplate, Tool, Prompt, MCPProps, MCPError, ServerInfo } from './types'
    ...
    public async connect() {
        ...
        this.eventSource.onerror = error => {
            ...
            if (this.reconnectAttempts < this.maxReconnectAttempts) {
                ...
                console.log(
                    `将在 ${delay}ms 后尝试重连 (${this.reconnectAttempts + 1}/${this.maxReconnectAttempts
                    })`
                )
        ...
        try {
            ...
            mcpClient = new MCPClient({
                ...
                onToolsReady: (tools) => {
                    if (mcpClient) {
                        cleanup()
                        let toolsFunctionCall = mcpClient.transformToolsToOpenAIFunctions(tools)
                        resolve({ tools, mcpClient, toolsFunctionCall });
                    }
                },
                onError: (error) => {
                    if (mcpClient) {
            ...
            reject(new Error(`Failed to initialize MCPClient: ${(error as Error).message}`));
        }
    })
        .catch((error) => {
            throw error;
